 ...
 *
 * <p>This interface is designed to provide a common protocol for objects that wish to execute code while they
 * are active. For example, {@code ThrowableRunnable} is implemented by class {@code Thread}. Being active simply
 * means that a thread has been started and has not yet been stopped.
 *
 * <p>In addition, {@code ThrowableRunnable} provides the means for a class to be active while not subclassing
